...
//    }
    ...
    public Person savePerson(@RequestBody PersonDto personDto){
        ...
        if (personDto.getCityId() != null){
            person.setCity(cityRepository.findById(personDto.getCityId()).orElseThrow(NoSuchElementException::new));
        }
        personRepository.save(person);
        return person;
    }
    ...
    public void deletePerson(@PathVariable long id){
        personRepository.delete(personRepository.findById(id).orElseThrow(NoSuchElementException::new));
    }
}
